...

{
	...
	if !(isNull _container) then {
		...
		_createdObjects pushback [_deployClass,_deployArea];
		_createdContainers pushback _container;
	};
} forEach _objectData;

_taskLogic setVariable ["deployableObjects",_createdObjects,!(isServer)];
_taskLogic setVariable ["deployableTargets",_createdContainers,!(isServer)];
