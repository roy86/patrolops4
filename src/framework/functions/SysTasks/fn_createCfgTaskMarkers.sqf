...
if (_areaSize isEqualTo []) then {[500,500]};

{
	...
	if (_markerPosition isEqualType "") then { _markerPosition = _markerPosition call BIS_fnc_position; };

	if !(_markerAngle isEqualType []) then { _markerAngle = [_markerAngle,_markerAngle]; };
	...
	if !(_markerDirection isEqualType []) then { _markerDirection = [_markerDirection,_markerDirection]; };
	...
	if !(_markerDistance isEqualType []) then { _markerDistance = [_markerDistance,_markerDistance]; };
