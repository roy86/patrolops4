...
params [["_object","",["",objNull]],["_fullPath",true,[false]],["_iconType","",[""]]];
...
private _typeSuffix = switch (true) do {
	case !(_iconType isEqualTo "") : {_iconType};
	case (_object isKindOf "CaManBase") : {"inf"};
	case (_object isKindOf "StaticMortar") : {"mortar"};
	//case (isText(configfile >> "CfgVehicles" >> typeof _object >> "incomingMissileDetectionSystem"));
	//case (isNumber(configfile >> "CfgVehicles" >> typeof _object >> "incomingMissileDetectionSystem")) : {"antiair"};
