...
switch (_mode) do {
	case "getCfgRoles" : {
		...
		private _roles = (["CfgRoles"] call MPSF_fnc_getCfgSubClasses) select {
            private _roleSideID = ["CfgRoles",_x,"side"] call MPSF_fnc_getCfgDataNumber;
	...
	case "getCfgRoleTraits" : {
		_params params [["_roleID","",[""]]];
		(["CfgRoles",_roleID,"traits"] call MPSF_fnc_getCfgDataArray) apply {toLower _x};
	};
    ...
    case "getTraits" : {
        (["CfgRoles","allTraits"] call MPSF_fnc_getCfgDataArray) apply {toLower _x};
    };
