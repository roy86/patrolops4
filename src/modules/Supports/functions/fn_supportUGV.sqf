...
switch (_mode) do {
	...
	case "airdrop" : {
		...
		driver _targetPlane forceSpeed 60;
		...
		_wp2 setWaypointStatements ["true",format ["_vehicle = %1 call BIS_fnc_objectFromNetID; {_vehicle deletevehiclecrew _x;} foreach crew _vehicle; deletevehicle _vehicle;",str _targetPlaneID]];
	...
	case "init" : {
		if (isServer) then {
			["MPSF_Server_onRequestSupplyDrop_EH","onRequestSupplyDrop",{
				...
				if ((missionNamespace getVariable ["MPSF_SupplyDrop_lastTime",MISSIONTIME]) <= MISSIONTIME) then {
					...
					["onRequestSupplyDropFailed",[_requestor],0] call MPSF_fnc_triggerEventhandler;
				};
			}] call MPSF_fnc_addEventHandler;
		};
        if (hasInterface) then {
			...
			["MPSF_Client_onRequestSupplyDropFailed_EH","onRequestSupplyDropFailed",{
				params [["_requestor",objNull,[objNull]]];
				if (_requestor isEqualTo player) then {
					player customChat [missionNamespace getVariable ["MPSF_Radio_channelID",-1],"Support Request Airdop is not available at this time."];
				};
			}] call MPSF_fnc_addEventHandler;
		};
	};
...
_position set [2,400];
private _ugv = objNull;
private _className = (["CfgMissionSupports","SupplyDrops","UGVs"] call MPSF_fnc_getCfgDataArray) param [(side player) call BIS_fnc_sideID,"",[""]];
if !(_className isEqualTo "") then {
